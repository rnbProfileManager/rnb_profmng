...
public class SelectProject{
	...
	public String selectProject(String projectCd, RedirectAttributes redirectAttributes) {
		...
		if(projectCd.equals("*")) {
			try {
				List result = projectService.selectAll();
				System.out.println(result);

			    if (result.size() > 0) {
			    	System.out.println("success");
			    	ProjectDTO projectDto = (ProjectDTO) result.get(0);
			        redirectAttributes.addFlashAttribute("selectAll", "success");
			        ...
			        redirectAttributes.addFlashAttribute("selectAll", "no Data");
			    }
			}catch (DuplicateKeyException e) {
		        // 중복 키 오류 발생 시
				System.out.println("duplicate");
		        redirectAttributes.addFlashAttribute("selectAll", "duplicate");
		    } catch (Exception e) {
		        // 기타 예외
		    	System.out.println(e);
		    	System.out.println("fail");
		        redirectAttributes.addFlashAttribute("selectAll", "fail");
		    }
